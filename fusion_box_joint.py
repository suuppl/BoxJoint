...
class BoxJointParameters:
    ...
    minFingers: Parameter = field(default_factory=lambda: Parameter(3))
    maxFingers: Parameter = field(default_factory=lambda: Parameter(33))
    minFingerWidth: Parameter = field(
        default_factory=lambda: Parameter.length(centimeters=1)
    )
    maxFingerWidth: Parameter = field(
        default_factory=lambda: Parameter.length(centimeters=15)
    )  # 15cm or 6"
    fingerRatio: Parameter = field(default_factory=lambda: Parameter(0.5))
    margin: Parameter = field(default_factory=lambda: Parameter.length(centimeters=0))
    bitDiameter: Parameter = field(
        default_factory=lambda: Parameter.length(centimeters=0)
    )  # 1/4"
    kerf: Parameter = field(default_factory=lambda: Parameter.length(centimeters=0))
...
class BoxJointAddIn(FusionCustomFeatureAddIn):
    ...
    def createInputs(self, command: adsk.core.Command, params: BoxJointParameters):
        ...
        facesSelectInput = inputs.addSelectionInput(
            "faces", "Faces", "Select outside faces of bodies to join."
        )
    ...
    def paramsToInputs(
        self, params: BoxJointParameters, commandInputs: adsk.core.CommandInputs
    ):
        ...
        commandInputs.itemById(
            "minFingerWidth"
        ).expression = params.minFingerWidth.expression
        commandInputs.itemById(
            "maxFingerWidth"
        ).expression = params.maxFingerWidth.expression
    ...
    def customFeatureToParams(
        self, feature: adsk.fusion.CustomFeature
    ) -> BoxJointParameters:
        customNamedValues = feature.customNamedValues
        parameters = feature.parameters

        faces = [EntityRef(token) for token in customNamedValues.value("faces").split()]
    ...
    def createOrUpdateChildFeatures(
        self,
...
def computeBoxJoint(params: BoxJointParameters) -> BaseCombines:
    ...
    for faceA, faceB, buttingFace in getAllButtingFaces(faces):
        ...
        tempBrepMgr.booleanOperation(
            overlap, bodyA, adsk.fusion.BooleanTypes.IntersectionBooleanType
        )
        ...
        fingers = min(
            math.floor(2 * (length / fingerAWidth - 1) / (1 + rBA)) | 1, maxFingers
        )
        ...
        finger = createSimpleBox(
            minX, minY, minZ, maxX - minX, maxY - minY, fingerBWidth
        )
        ...
        if isObtuse:
            ...
            try:
                ...
                tempBrepMgr.booleanOperation(
                    finger, slope, adsk.fusion.BooleanTypes.DifferenceBooleanType
                )
            ...
            try:
                ...
                tempBrepMgr.booleanOperation(
                    finger, slope, adsk.fusion.BooleanTypes.UnionBooleanType
                )
                slope = createPrism(
                    slopeCrossSection, fingerBWidth + fingerAWidth + margin
                )
                tempBrepMgr.booleanOperation(
                    finger, slope, adsk.fusion.BooleanTypes.UnionBooleanType
                )
        ...
        if bitRadius > 0:
            ...
            tempBrepMgr.booleanOperation(
                fingerACutter, cove, adsk.fusion.BooleanTypes.UnionBooleanType
            )
            ...
            tempBrepMgr.booleanOperation(
                fingerBJoiner, cove, adsk.fusion.BooleanTypes.UnionBooleanType
            )
            tempBrepMgr.transform(cove, translateUp)
            tempBrepMgr.booleanOperation(
                fingerBJoiner, cove, adsk.fusion.BooleanTypes.UnionBooleanType
            ...
            tempBrepMgr.booleanOperation(
                fingerACutter, cove, adsk.fusion.BooleanTypes.DifferenceBooleanType
            )
            tempBrepMgr.transform(cove, translateUp)
            tempBrepMgr.booleanOperation(
                fingerACutter, cove, adsk.fusion.BooleanTypes.DifferenceBooleanType
            ...
            tempBrepMgr.booleanOperation(
                fingerBJoiner, cove, adsk.fusion.BooleanTypes.DifferenceBooleanType
            )
            tempBrepMgr.transform(cove, translateUp)
            tempBrepMgr.booleanOperation(
                fingerBJoiner, cove, adsk.fusion.BooleanTypes.DifferenceBooleanType
            ...
            dogBoneCrossSection = createFaceFromCurves(
                [
                    adsk.core.Circle3D.createByCenter(
                        center=pIc, normal=vOAPerp, radius=bitRadius
                    ),
                ]
            )
            dogBone = createObliquePrism(dogBoneCrossSection, vAeI)
            tempBrepMgr.booleanOperation(
                fingerACutter, dogBone, adsk.fusion.BooleanTypes.UnionBooleanType
            )
            ...
            dogBoneCrossSection = createFaceFromCurves(
                [
                    adsk.core.Circle3D.createByCenter(
                        center=pIc, normal=vOBPerp, radius=bitRadius
                    ),
                ]
            )
            dogBone = createObliquePrism(dogBoneCrossSection, vBeI)
            tempBrepMgr.booleanOperation(
                fingerBJoiner, dogBone, adsk.fusion.BooleanTypes.DifferenceBooleanType
            )
            tempBrepMgr.transform(dogBone, translateUp)
            tempBrepMgr.booleanOperation(
                fingerBJoiner, dogBone, adsk.fusion.BooleanTypes.DifferenceBooleanType
...
def getAllButtingFaces(
    outsideFaces: list[adsk.fusion.BRepFace],
) -> list[tuple[adsk.fusion.BRepFace, adsk.fusion.BRepFace, adsk.fusion.BRepFace]]:
    buttingFaces = []
...
def getButtingFaces(
    faceA: adsk.fusion.BRepFace, faceB: adsk.fusion.BRepFace
    ...
    for edgeOnFaceB in faceB.edges:
        for candidateButtingFace in edgeOnFaceB.faces:
            ...
            if (
                candidateButtingFaceGeometry.surfaceType
            ...
            for f in faceA.body.shells[0].faces:
                ...
                imprint = tempBrepMgr.imprintOverlapBodies(
                    tempBrepMgr.copy(candidateButtingFace), tempBrepMgr.copy(f), False
                )
                if imprint[3].size() > 0:
